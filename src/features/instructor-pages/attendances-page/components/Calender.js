...
import {
  MenuItem, Select, FormControl, Typography,
  Card, CardContent, Grid, useMediaQuery, Box
} from '@mui/material';
...
function InstructorAttendance({attendanceData}) {
  ...
  const formatAttendanceForMonth = (attendanceData, month) => {
    ...
    return attendanceData
      .filter(({ date }) => new Date(date).getMonth() === month && new Date(date).getFullYear() === year)
      .map(({ date, status }) => ({
  ...
  useEffect(() => {
    ...
    const attendanceObject = formattedAttendance.reduce((acc, { date, status }) => {
      acc[date] = status;
      return acc;
    }, {});
        ...
        <Grid item xs={2.4} key={i}>
          <Card>
            <CardContent>
              ...
              <Typography sx={{fontSize:"21px",fontWeight:"300",textAlign:"end"}} >{i}</Typography>
              <Button
                sx={{backgroundColor : attendanceStatus === 'Present' ? "#14BC10" : "#FF4B4B", padding:"0px", color:"white"}}
          ...
          <Grid item style={{ marginBottom: '20px', display: "flex", alignItems: "center" }}>
            ...
            <FormControl style={{ marginLeft: '5px' }}>
              <Select
                ...
                size='small'
                sx={{
                  color: "#5611B1", backgroundColor: "#DFC7FF", fontSize: "16px",
                  fontWeight: 600, lineHeight: "24px", minWidth: "89px", maxWidth: "89px"
                }}
                variant='outlined'
              >
