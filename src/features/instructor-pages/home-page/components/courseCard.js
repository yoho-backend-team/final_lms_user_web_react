    ...
    <StyledPaper>
      ...
      <Box sx={{ position: 'absolute', top: '66px', left: '19px' }}>
        {batches.map((batch, index) => (
          <SlantedBox key={index} backgroundColor={batch.backgroundColor} sx={{ marginBottom: '16px', width: '321px', height: '89px' }}>
            <Typography
              sx={{
                position: 'absolute',
                top: '24px',
            ...
            <ProgressCircleContainer>
              <CircularProgress
                variant="determinate"
                value={100}
                sx={{ color: batch.backgroundColor, zIndex: 1, position: 'absolute' }}
                size={67.01}
              />
              <ProgressCircle
                variant="determinate"
