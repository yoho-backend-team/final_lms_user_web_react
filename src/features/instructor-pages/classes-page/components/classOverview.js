...
import { Card, CardContent, Typography, Box, Button, Grid, Breadcrumbs, Link, AvatarGroup,Avatar,Tooltip } from '@mui/material';
...
const ClassCard = ({type,classDetails,getClass}) => {
  ...
  return (
    ...
    {!showAttendance ? 
      <Box sx={{ boxShadow: '0px 0px 64px 0px rgba(0, 0, 0, 0.14)', borderRadius: "10px", border: "1px solid #C3C3C3", backgroundColor: "white",  height : "100vh", overflow:"auto" }}>
        <Box sx={{ padding: "20px" }}>
          <Box sx={{ display: "flex", gap: "24px", mb: 3 }}>
            ...
            <Box sx={{ display: "inline-flex", alignItems: "center" }}>
              ...
              <Typography sx={{ color: "#495057", fontSize: "16px", fontWeight: 600, lineHeight: "24px" }}>#{classDetails?.batch?.id}</Typography>
            </Box>
          </Box>
                ...
                <Box sx={{ display: 'flex',justifyContent:"flex-start", mb: 2}}>
                  <Box sx={{gap: "40px",display:"flex", backgroundColor: "#DFC7FF", borderRadius: "16px", padding: "26px 38px"}} >
                    <Box>
                      ...
                      <Typography variant="body1" sx={{ fontWeight: 500, fontSize: "16px", lineHeight: "24px", color: "#7149A5" }}>{ formatDate(classDetails?.start_date)}</Typography>
                    </Box>
                    <Box>
                      <Typography variant="body2" sx={{ color: "#000", fontSize: "16px", fontWeight: 600, lineHeight: "16px", pb: "6px" }}>Start At</Typography>
                      <Typography variant="body1" sx={{ fontWeight: 500, fontSize: "16px", lineHeight: "24px", color: "#7149A5" }}>{formatTime(classDetails?.start_time)}</Typography>
                    </Box>
                    <Box>
                      <Typography variant="body2" sx={{ color: "#000", fontSize: "16px", fontWeight: 600, lineHeight: "16px", pb: "6px" }}>End At</Typography>
                      <Typography variant="body1" sx={{ fontWeight: 500, fontSize: "16px", lineHeight: "24px", color: "#7149A5" }}>{formatTime(classDetails?.end_time)}</Typography>
                    </Box>
                  </Box>
                </Box>
                <Typography sx={{ color: "#495057", fontSize: "20px", fontWeight: 800, lineHeight: "32px", pb: "10px" }}>Class Meeting Link</Typography>
                <Typography sx={{ color: "#828282", fontSize: "14px", fontWeight: 600, lineHeight: "32px" }}>Join the class @{formatTime(classDetails?.start_time)}</Typography>
            ...
            <Grid item xs={8} md={6}>
              <Box>
                ...
                <Box sx={{ display: 'flex', gap: "40px", mb: 2 }}>
                    ...
                    <Box sx={{ px:"20px" }} >
                       <AvatarGroup total={classDetails?.batch?.student?.length} >
                       {
                        classDetails?.batch?.student?.map((user)=>
                          <Tooltip title={user?.full_name} key={user?.full_name}  >
                              <Avatar src={ user?.image ? getImageUrl(user?.image) : profilePlaceholder } />
                          </Tooltip>
                        )
                       }
                       </AvatarGroup>
                    </Box>
                    <Typography variant="body1" sx={{ fontWeight: '500',fontSize:"12px",lineHeight:"16px",color:"#435D85",p:"20px" }}> Total {classDetails?.batch?.student?.length}</Typography>
                  </Box>
                  <Box
                   sx={{
                  ...
                  >
                    <Typography variant="body2" sx={{ color : "#000000", fontSize: "14px", fontWeight: 600, lineHeight: "16px",p:"20px" }} >Instructors</Typography>
                    <Box sx={{ px:"20px"}} >
                    ...
                    <Typography variant="body1" sx={{ fontWeight: '500',fontSize:"12px",lineHeight:"16px",color:"#435D85",p:"20px" }} > Total {classDetails?.instructors?.length}</Typography>
                  </Box>
                </Box>
                <Box 
                sx={{ 
                ...
                >
                  ...
                  <Box sx={{padding:"20px"}} >
                  <Typography variant="body2" sx={{ color : "#000000", fontSize: "14px", fontWeight: 600, lineHeight: "16px",pb:"10px"}} >Student Details</Typography>
                    <AvatarGroup total={classDetails?.batch?.student?.length} >
                       {
                        classDetails?.batch?.student?.map((user)=>
                          <Tooltip title={user?.full_name} key={user?.full_name}  >
                    ...
                    <Typography sx={{ color : "#91642E", fontWeight:500,fontSize:"12px",lineHeight:"16px", pt: "10px" }} >Total {classDetails?.batch?.student?.length}</Typography>
                  </Box>
      ...
      :
      <Attendance setShowAttendance={setShowAttendance} classDetails={classDetails} />
    }
    </Box>
  );
};
