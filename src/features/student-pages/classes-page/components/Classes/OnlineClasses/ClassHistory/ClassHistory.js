...
function StudentOnlineClassHistory() {
  ...
  const months = classHistory.map((item) => new Date(item.date).toLocaleString('default', { month: 'long' })).filter(unique);
  const years = classHistory.map((item) => new Date(item.date).getFullYear()).filter(unique);

  return (
    <>
      <Grid container spacing={2}>
        <Grid item>
          <FormControl sx={{ m: 1, minWidth: 140, color: 'var(--Gray-700, #495057)', fontFamily: 'Poppins', fontSize: '12px', fontWeight: 500, lineHeight: '16px' }}>
              ...
              <Box sx={{
                ...
                <Box sx={{ marginBottom: matches ? 0 : 2 }}>
                  <Typography variant="body2" sx={{ color: 'var(--Gray-600, #6C757D)', fontFamily: 'Poppins', fontSize: '12px', fontWeight: 500, lineHeight: '22px' }}>
                    <PeopleAltIcon sx={{ marginRight: '8px' }} /> {item.classNumber}
                  </Typography>
                </Box>
                <Box sx={{ marginBottom: matches ? 0 : 2 }}>
                  <Typography variant="body2" sx={{ color: 'var(--Gray-600, #6C757D)', fontFamily: 'Poppins', fontSize: '12px', fontWeight: 500, lineHeight: '22px' }}>
