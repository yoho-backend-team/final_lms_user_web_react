...
const CourseCard = () => {
  ...
  // Extract progress data, default to 0% if no data is available
  const progress = reports?.classes?.[0]?.total ?? 0;
  ...
  const options = {
    ...
    series: [progress], // Ensure it defaults to 0%
    plotOptions: {
      radialBar: {
        hollow: {
          margin: 15,
        ...
        dataLabels: {
          ...
          value: {
            ...
            fontWeight: 800,
            show: true,
            formatter: (val) => `${val}%`, // Display percentage format
          },
        },
        track: {
